...
def save_indexed_dict(path: str, mode: str , dataset_name: str, i: int, EnergyFunction: str, seed: int, indexed_solution_dict):
    ...
    newpath = path + f"/DatasetCreator/loadGraphDatasets/DatasetSolutions/no_norm/{dataset_name}/{mode}/{seed}/{EnergyFunction}/indexed"
    if not os.path.exists(newpath):
        os.makedirs(newpath)

    save_path = path + f"/DatasetCreator/loadGraphDatasets/DatasetSolutions/no_norm/{dataset_name}/{mode}/{seed}/{EnergyFunction}/indexed/idx_{i}_solutions.pickle"
